...
class EEGDataset(Dataset):
    def __init__(self, eeg, emg, kin):
        ...
        assert emg.shape[0] == kin.shape[0], "EMG and KIN not equal"
        self.eeg_data = eeg
        self.emg_data = emg
        self.kin_data = kin
...
def train(model, train_loader, test_loader, optimizer, criterion, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for eeg, emg, kin in tqdm(train_loader, desc=f'epoch:{epoch+1}/{num_epochs}',unit='batch'):
            ...
            optimizer.zero_grad()
            outputs = model(eeg_batch, emg_batch)  # (B, 6)
        ...
        epoch_loss = running_loss / len(train_loader.dataset)  # loss epoch
...
def evaluate(model, test_loader, device):
    ...
    with torch.no_grad():
        for eeg, emg, kin in test_loader:
