...
class MutiCovnet(nn.Module):
    def __init__(self): #输入 (32, 250)的EEG数据，输出xyz的坐标
        ...
        # 将四个分支的输出通道数拼接，32*2=64
        self.conv_fusion = nn.Conv2d(64, 64, kernel_size=1, bias=False)
        self.bn_fusion = nn.BatchNorm2d(64)
        # (32, 2*32, 250)
    ...
    def forward(self, x):  # x: (B, 250, 32)
        ...
        x = torch.cat([b1, b2], dim=1) #(B, 64, 32, 250)
...
class Embedding(nn.Module):
    def __init__(self, in_channels=32, in_features=64, d_model=32):
        super().__init__()
...
class Connected_layer(nn.Module):
    def __init__(self, input_dim=32, hidden_dim=128, output_dim=6):
        super().__init__()
...
class EEGTransformerModel(nn.Module):
    def __init__(self, output_dim=6):
        ...
        self.attention = Muti_Attention(dim=32)
